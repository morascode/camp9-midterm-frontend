...
import { usePatchBookmark } from '../hooks/useBookmarks';
...
function MovieDetails() {
  ...
  const patchBookmark = usePatchBookmark(movieId);
  const { data, isLoading, isError, isFetching } = useGetMovieDetails(movieId);
  const isBookmarked = data && data.bookmarkedBy.length > 0;
  if (isLoading) {
    return (
    ...
    return (
      <article className="h-screen w-screen flex flex-col pb-6">
        <HeaderPage
          ...
          hasHeartButton={true}
          isLoading={patchBookmark.isLoading || isFetching}
          onHeartButtonClick={() => {
            if (isBookmarked !== undefined) patchBookmark.mutate(isBookmarked);
          }}
        />
